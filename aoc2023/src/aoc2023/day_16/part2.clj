...
(defn sides-dirs [grid]
  (concat
    (for [x (range (:X grid)) y [-1 (:Y grid)]] (list [x y] (if (= y -1) dir-d dir-u)))
    (for [y (range (:Y grid)) x [-1 (:X grid)]] (list [x y] (if (= x -1) dir-r dir-l)))))
...
(defn -main
  ...
  (with-open [rdr (clojure.java.io/reader *in*)]
    (let [grid (process (line-seq rdr))
          counts (for [[start dir] (sides-dirs grid)
                       :let [c (count (track-beams grid [start dir] [] (new-visited)))
                             _ (println start dir c)]]
                   c)]
      (dorun counts)
      (printf "Max is: %d\n" (apply max counts))
      ))
