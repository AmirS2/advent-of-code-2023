...
(defn smudge-reflection [above below]
  (let [a (apply concat above)
        b (apply concat below)
        non-matches (count (filter not (map = a b)))]
    (= non-matches 1)))
...
(defn find-horiz-mirror [field]
  (let [sz (count field)
        ...
        mirror-pos (for [i (range 1 sz)
                         ...
                         :when (smudge-reflection above below)]
                     i)]
